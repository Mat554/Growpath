window.globalQuestionsData=window.globalQuestionsData||[];window.selectedQuestionIds=new Set;window.currentPublisherFolder=null;document.addEventListener("DOMContentLoaded",()=>{window.loadQuestionArchive(),window.initPublisherFolders()});window.initPublisherFolders=function(){window.updatePublisherFolderCounts()};window.updatePublisherFolderCounts=function(){const e={10:0,11:0,12:0};window.globalQuestionsData.forEach(t=>{t.is_active==1&&t.target_class&&t.target_class.split(",").map(n=>n.trim()).forEach(n=>{e[n]!==void 0&&e[n]++})}),document.getElementById("folder-count-10").innerText=e[10]+" soal",document.getElementById("folder-count-11").innerText=e[11]+" soal",document.getElementById("folder-count-12").innerText=e[12]+" soal"};window.openPublisherFolder=function(e){window.currentPublisherFolder=e,["10","11","12"].forEach(r=>{const l=document.getElementById("folder-btn-"+r);if(l)if(r===e){const a={10:"blue",11:"green",12:"purple"}[r];l.className=`flex-1 bg-${a}-100 hover:bg-${a}-200 border-2 border-${a}-400 rounded-xl p-4 transition-all shadow-sm`}else{const a={10:"blue",11:"green",12:"purple"}[r];l.className=`flex-1 bg-${a}-50 hover:bg-${a}-100 border-2 border-${a}-200 rounded-xl p-4 transition-all`}});const t=document.getElementById("publisherFolderTitle"),s={10:"blue",11:"green",12:"purple"},n={10:"Kelas 10",11:"Kelas 11",12:"Kelas 12"};t&&(t.innerHTML=`<span class="text-${s[e]}-600 font-bold">${n[e]}</span>`);const o=document.getElementById("publisherFolderActions");o&&o.classList.remove("hidden");const i=document.getElementById("publisherFolderActions");i&&(i.className="gap-2 flex"),window.loadPublisherQuestions(e)};window.closePublisherFolder=function(){window.currentPublisherFolder=null,["10","11","12"].forEach(n=>{const o=document.getElementById("folder-btn-"+n);if(o){const r={10:"blue",11:"green",12:"purple"}[n];o.className=`flex-1 bg-${r}-50 hover:bg-${r}-100 border-2 border-${r}-200 rounded-xl p-4 transition-all`}});const e=document.getElementById("publisherFolderTitle");e&&(e.innerText="Pilih folder di atas");const t=document.getElementById("publisherFolderActions");t&&t.classList.add("hidden");const s=document.getElementById("publisherList");s&&(s.innerHTML=`
            <div class="text-center p-8 text-gray-400">
                <i class="ph-fill ph-folder-open text-5xl mb-3 text-gray-300"></i>
                <p>Klik folder di atas untuk melihat soal</p>
            </div>
        `)};window.loadPublisherQuestions=function(e){const t=document.getElementById("publisherList");if(!t)return;t.innerHTML="";const s=window.globalQuestionsData.filter(n=>n.is_active!=1||!n.target_class?!1:n.target_class.split(",").map(i=>i.trim()).includes(e));if(s.length===0){const o={10:"blue",11:"green",12:"purple"}[e];t.innerHTML=`
            <div class="text-center p-8 text-gray-400">
                <i class="ph-fill ph-folder-simple-dashed text-5xl mb-3 text-${o}-200"></i>
                <p>Belum ada soal untuk Kelas ${e}</p>
                <small class="text-xs">Tambahkan soal di menu Kelola Soal</small>
            </div>
        `;return}s.forEach(n=>{const o=window.selectedQuestionIds.has(n.id),r={10:"blue",11:"green",12:"purple"}[e],l=o?`border-${r}-400 bg-${r}-50`:"border-gray-200 bg-white hover:border-gray-300 hover:bg-gray-50",d=o?`text-${r}-500`:"text-gray-300",a=o?"ph-fill ph-check-square":"ph ph-square",c=`
        ...
        </div>`;t.innerHTML+=c}),window.updateTotalSelected()};window.selectAllInFolder=function(){if(!window.currentPublisherFolder)return;window.globalQuestionsData.filter(t=>t.is_active!=1||!t.target_class?!1:t.target_class.split(",").map(n=>n.trim()).includes(window.currentPublisherFolder)).forEach(t=>{window.selectedQuestionIds.add(t.id)}),window.loadPublisherQuestions(window.currentPublisherFolder)};window.clearFolderSelection=function(){if(!window.currentPublisherFolder)return;window.globalQuestionsData.filter(t=>!window.selectedQuestionIds.has(t.id)||!t.target_class?!1:t.target_class.split(",").map(n=>n.trim()).includes(window.currentPublisherFolder)).forEach(t=>{window.selectedQuestionIds.delete(t.id)}),window.loadPublisherQuestions(window.currentPublisherFolder)};window.updateTotalSelected=function(){const e=document.getElementById("totalSelected");e&&(e.innerText=window.selectedQuestionIds.size+" Item")};window.toggleSelection=function(e){window.selectedQuestionIds.has(e)?window.selectedQuestionIds.delete(e):window.selectedQuestionIds.add(e),window.currentPublisherFolder&&window.loadPublisherQuestions(window.currentPublisherFolder),window.updateTotalSelected()};window.loadQuestionArchive=function(){const e={all:[],10:[],11:[],12:[]};window.globalQuestionsData.forEach(t=>{const s=t.target_class?t.target_class.split(",").map(n=>n.trim()):[];e.all.push(t),s.forEach(n=>{e[n]&&e[n].push(t)})}),window.renderArchiveColumn("10",e[10]),window.renderArchiveColumn("11",e[11]),window.renderArchiveColumn("12",e[12]),window.renderAllQuestionsList(e.all),document.getElementById("count-class-10").innerText=e[10].length,document.getElementById("count-class-11").innerText=e[11].length,document.getElementById("count-class-12").innerText=e[12].length,document.getElementById("count-all").innerText=e.all.length,window.updatePublisherFolderCounts()};window.renderArchiveColumn=function(e,t){const s=document.getElementById("archive-class-"+e);if(s){if(t.length===0){s.innerHTML='<div class="text-center py-8 text-gray-400 text-sm">Seret soal ke sini</div>';return}s.innerHTML=t.map(n=>window.createArchiveCard(n,e)).join("")}};window.renderAllQuestionsList=function(e){const t=document.getElementById("allQuestionsList");if(t){if(e.length===0){t.innerHTML='<div class="text-center py-8 text-gray-400 text-sm">Belum ada soal.</div>';return}t.innerHTML=e.map(s=>window.createAllQuestionsCard(s)).join("")}};window.createArchiveCard=function(e,t){const n={10:"blue",11:"green",12:"purple"}[t]||"gray",o=e.is_active==1;return`
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center gap-2 mt-2 flex-wrap">
                        ...
                        <button onclick="window.toggleQuestionStatus(${e.id})"
                                class="px-2 py-0.5 rounded text-[10px] font-semibold transition-all ${o?"bg-green-500 hover:bg-green-600 text-white":"bg-gray-300 hover:bg-gray-400 text-gray-600"}">
                            <i class="ph-fill ph-power ${o?"":"hidden"}"></i>
                            <i class="ph ph-power ${o?"hidden":""}"></i>
                            ${o?"Aktif":"Draft"}
                        </button>
                    </div>
    ...
    `};window.createAllQuestionsCard=function(e){const t=e.target_class?e.target_class.split(",").map(i=>i.trim()):[],s=t.length>0?t.map(i=>`<span class="px-2 py-0.5 bg-[#EBF5FF] text-[#4A90E2] rounded text-[10px] font-semibold">K${i}</span>`).join(""):'<span class="px-2 py-0.5 bg-gray-100 text-gray-400 rounded text-[10px] font-semibold">-</span>',n=e.is_active==1,o=t.length>0;return`
    ...
    `};window.draggedQuestionId=null;window.draggedFromClass=null;window.handleArchiveDragStart=function(e,t){window.draggedQuestionId=t,e.target.classList.add("opacity-50","scale-95"),e.dataTransfer.effectAllowed="move",e.dataTransfer.setData("text/plain",t)};window.handleArchiveDragEnd=function(e){e.target.classList.remove("opacity-50","scale-95"),window.draggedQuestionId=null,window.draggedFromClass=null,document.querySelectorAll(".drag-over").forEach(t=>{t.classList.remove("drag-over","border-solid")})};window.handleColumnDragOver=function(e){e.preventDefault(),e.dataTransfer.dropEffect="move",e.currentTarget.classList.add("drag-over")};window.handleColumnDragLeave=function(e){e.currentTarget.classList.remove("drag-over")};window.handleColumnDrop=function(e,t){if(e.preventDefault(),e.currentTarget.classList.remove("drag-over"),!window.draggedQuestionId)return;const s=window.draggedQuestionId,n=window.globalQuestionsData.find(r=>r.id==s);if(!n)return;let o=n.target_class?n.target_class.split(",").map(r=>r.trim()):[];o.includes(t)||o.push(t),o.sort((r,l)=>r-l);const i=o.join(",");window.updateQuestionClass(s,i)};window.updateQuestionClass=function(e,t){fetch(`/admin-dashboard/question/${e}/class`,{method:"POST",headers:{"Content-Type":"application/json","X-CSRF-TOKEN":window.csrfToken,Accept:"application/json"},body:JSON.stringify({target_class:t})}).then(async s=>{if(!s.ok){const n=await s.json().catch(()=>({}));throw new Error(n.message||"Gagal mengupdate kelas soal.")}return s.json()}).then(s=>{if(s.success){const n=window.globalQuestionsData.find(o=>o.id==e);n&&(n.target_class=t),window.loadQuestionArchive()}}).catch(s=>{console.error("Error updating class:",s),alert("Gagal mengupdate kelas: "+s.message)})};window.toggleQuestionStatus=function(e){fetch(`/admin-dashboard/question/${e}/toggle`,{method:"POST",headers:{"X-CSRF-TOKEN":window.csrfToken,Accept:"application/json"}}).then(t=>t.json()).then(t=>{if(t.success){const s=window.globalQuestionsData.find(n=>n.id==e);s&&(s.is_active=s.is_active==1?0:1),window.loadQuestionArchive()}}).catch(t=>{console.error("Error toggling status:",t)})};window.removeFromClass=function(e,t){const s=window.globalQuestionsData.find(i=>i.id==e);if(!s)return;let n=s.target_class?s.target_class.split(",").map(i=>i.trim()):[];n=n.filter(i=>i!==t);const o=n.join(",");window.updateQuestionClass(e,o)};window.removeFromAllClasses=function(e){confirm("Hapus soal dari semua kelas? Soal akan tetap Aktif tapi tidak ada di arsip manapun.")&&window.updateQuestionClass(e,"")};window.removeFromActive=function(e){confirm("Hapus soal dari status Aktif? Soal akan menjadi Draft.")&&fetch(`/admin-dashboard/question/${e}/remove`,{method:"POST",headers:{"X-CSRF-TOKEN":window.csrfToken,Accept:"application/json"}}).then(t=>t.json()).then(t=>{if(t.success){const s=window.globalQuestionsData.find(n=>n.id==e);s&&(s.is_active=0),window.loadQuestionArchive()}}).catch(t=>{console.error("Error removing from active:",t),alert("Gagal menghapus dari aktif.")})};window.deleteQuestion=function(e){confirm("Yakin ingin menghapus soal ini? Tindakan ini tidak dapat dibatalkan.")&&fetch(`/admin-dashboard/question/${e}/delete`,{method:"POST",headers:{"X-CSRF-TOKEN":window.csrfToken,Accept:"application/json"}}).then(t=>t.json()).then(t=>{t.success&&(window.globalQuestionsData=window.globalQuestionsData.filter(s=>s.id!=e),window.selectedQuestionIds.delete(e),window.loadQuestionArchive(),window.updatePublisherFolderCounts())}).catch(t=>{console.error("Error deleting question:",t),alert("Gagal menghapus soal.")})};
