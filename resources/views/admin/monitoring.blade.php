...
<body class="bg-[#F4F7F6] font-sans text-[#333]">
    ...
    <aside id="mobileSidebar" class="mobile-menu fixed top-0 left-0 w-[260px] h-full bg-white z-50 flex flex-col transform -translate-x-full overflow-y-auto">
        ...
        <div class="flex-1 flex flex-col gap-1 p-4 overflow-y-auto">
            ...
            <a href="{{ route('admin.dashboard') }}" onclick="closeMobileMenu()" class="w-full flex items-center gap-3 px-4 py-3 text-gray-500 hover:bg-gray-50 rounded-xl font-medium transition-all text-left">
                <i class="ph ph-squares-four text-lg"></i> Dashboard
            </a>

            <div class="text-xs font-semibold text-gray-400 uppercase tracking-wider mt-6 mb-2 pl-3">Laporan</div>
            <a href="{{ route('admin.monitoring') }}" onclick="closeMobileMenu()" class="w-full flex items-center gap-3 px-4 py-3 text-[#4A90E2] bg-[#EBF5FF] rounded-xl font-medium transition-all text-left">
    ...
    <main class="md:ml-[260px] p-4 md:p-8 pb-24 md:pb-8 overflow-y-auto bg-[#F4F7F6]">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8 gap-4">
            <div>
                <h2 class="text-xl md:text-2xl font-semibold text-gray-800">Monitoring Ujian</h2>
        ...
        <div class="bg-white p-4 md:p-8 rounded-2xl shadow-sm">
            ...
            <div class="mb-4 md:mb-6 flex flex-col md:flex-row items-start md:items-center gap-4">
                <label class="text-sm font-medium text-gray-600">Pilih Ujian:</label>
                <select id="examFilter" onchange="fetchMonitoringData()" class="px-4 py-2 border border-gray-200 rounded-lg text-sm focus:border-[#4A90E2] outline-none w-full md:min-w-[250px]">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr>
                            <th class="p-3 md:p-4 border-b-2 border-gray-100 text-gray-500 font-semibold text-xs md:text-sm">Nama Siswa</th>
                            <th class="p-3 md:p-4 border-b-2 border-gray-100 text-gray-500 font-semibold text-xs md:text-sm">Kelas</th>
                            <th class="p-3 md:p-4 border-b-2 border-gray-100 text-gray-500 font-semibold text-xs md:text-sm">Ujian</th>
                            <th class="p-3 md:p-4 border-b-2 border-gray-100 text-gray-500 font-semibold text-xs md:text-sm">Status</th>
                            <th class="p-3 md:p-4 border-b-2 border-gray-100 text-gray-500 font-semibold text-xs md:text-sm">Progres</th>
                        </tr>
                    </thead>
                ...
                .then(data => {
                    ...
                    data.forEach(item => {
                        ...
                        if(item.status === 'Selesai') {
                            statusBadge = '<span class="px-2 md:px-3 py-1 bg-[#E8F9F5] text-[#2ECC71] rounded-full text-[10px] md:text-xs font-bold uppercase border border-green-100"><i class="ph-bold ph-check"></i> Selesai</span>';
                        } else if(item.status === 'Failed') {
                            statusBadge = '<span class="px-2 md:px-3 py-1 bg-red-50 text-red-500 rounded-full text-[10px] md:text-xs font-bold uppercase border border-red-100"><i class="ph-bold ph-x"></i> Failed</span>';
                        } else {
                            statusBadge = '<span class="px-2 md:px-3 py-1 bg-[#FFF4E5] text-[#FF9F43] rounded-full text-[10px] md:text-xs font-bold uppercase border border-orange-100"><i class="ph-bold ph-spinner animate-spin"></i> Mengerjakan</span>';
                        }

                        tbody.innerHTML += `
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="p-3 md:p-4 border-b border-gray-50 font-medium text-gray-800 text-sm">${item.student_name}</td>
                                <td class="p-3 md:p-4 border-b border-gray-50 text-gray-600 text-sm">${item.student_class}</td>
                                <td class="p-3 md:p-4 border-b border-gray-50 text-gray-600 text-sm">${item.exam_title}</td>
                                <td class="p-3 md:p-4 border-b border-gray-50">${statusBadge}</td>
                                <td class="p-3 md:p-4 border-b border-gray-50 text-gray-600 font-semibold text-sm">${item.progress}</td>
                            </tr>
                        `;
                    });
                })
