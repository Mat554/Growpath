...
<head>
    ...
    <style>
        /* Menghapus font-family manual agar mengikuti Tailwind (font-sans) seperti dashboard */
    ...
    <main class="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <div class="p-4 md:p-8 pb-0 md:pb-0 shrink-0">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                ...
                <div class="w-full md:w-auto relative">
                    <input type="text" id="searchInput" onkeyup="searchKuesioner()" placeholder="Cari kuesioner..." class="w-full md:w-64 pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#4A90E2] focus:ring-4 focus:ring-[#4A90E2]/10 transition-all font-sans">
                    <i class="ph ph-magnifying-glass absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400 text-lg"></i>
                </div>
            </div>

            <div class="flex gap-2 border-b border-gray-200 pb-px overflow-x-auto hide-scrollbar" id="filterTabs">
                <button onclick="filterKuesioner('semua', this)" class="tab-btn active px-5 py-2.5 text-sm font-semibold text-[#4A90E2] border-b-2 border-[#4A90E2] whitespace-nowrap transition-all cursor-pointer bg-transparent font-sans">Semua Kuesioner</button>
                <button onclick="filterKuesioner('belum', this)" class="tab-btn px-5 py-2.5 text-sm font-medium text-gray-500 hover:text-gray-700 border-b-2 border-transparent whitespace-nowrap transition-all cursor-pointer bg-transparent font-sans">Belum Dikerjakan</button>
                <button onclick="filterKuesioner('selesai', this)" class="tab-btn px-5 py-2.5 text-sm font-medium text-gray-500 hover:text-gray-700 border-b-2 border-transparent whitespace-nowrap transition-all cursor-pointer bg-transparent font-sans">Selesai</button>
            </div>
        </div>
        ...
        <div class="flex-1 p-4 md:p-8 pt-6 md:pt-6 overflow-y-auto pb-24 md:pb-8">
            <div class="flex flex-col gap-4" id="kuesionerContainer">
                ...
                <div class="kuesioner-card bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-5 items-start md:items-center hover:-translate-y-1 transition-transform duration-300 {{ ($isOverdue && !$isCompleted) ? 'opacity-75 grayscale-[20%]' : '' }}" data-kategori="{{ $kategoriFilter }}">
                    ...
                    <div class="flex-1 w-full">
                        <div class="flex items-center gap-3 mb-1.5">
                            ...
                            @if($isCompleted)
                                <span class="hidden md:inline-block px-3 py-1 bg-[#E8F9F5] text-[#2ECC71] rounded-full text-[10px] font-bold uppercase tracking-wider border border-green-100">Selesai</span>
                            @elseif($isLocked)
                                <span class="hidden md:inline-block px-3 py-1 bg-gray-100 text-gray-500 rounded-full text-[10px] font-bold uppercase tracking-wider border border-gray-200">Belum Dibuka</span>
                            @elseif($isOverdue)
                                <span class="hidden md:inline-block px-3 py-1 bg-red-50 text-red-500 rounded-full text-[10px] font-bold uppercase tracking-wider border border-red-100">Ditutup</span>
                            @else
                                <span class="hidden md:inline-block px-3 py-1 bg-[#FFF4E5] text-[#FF9F43] rounded-full text-[10px] font-bold uppercase tracking-wider border border-orange-100">Belum Dikerjakan</span>
                            @endif
                        </div>
                        ...
                        <div class="mt-3 md:hidden">
                            @if($isCompleted)
                                <span class="inline-block px-3 py-1 bg-[#E8F9F5] text-[#2ECC71] rounded-full text-[10px] font-bold uppercase tracking-wider border border-green-100">Selesai</span>
                            @elseif($isLocked)
                                <span class="inline-block px-3 py-1 bg-gray-100 text-gray-500 rounded-full text-[10px] font-bold uppercase tracking-wider border border-gray-200">Belum Dibuka</span>
                            @elseif($isOverdue)
                                <span class="inline-block px-3 py-1 bg-red-50 text-red-500 rounded-full text-[10px] font-bold uppercase tracking-wider border border-red-100">Ditutup</span>
                            @else
                                <span class="inline-block px-3 py-1 bg-[#FFF4E5] text-[#FF9F43] rounded-full text-[10px] font-bold uppercase tracking-wider border border-orange-100">Belum Dikerjakan</span>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="w-full md:w-auto mt-4 md:mt-0 shrink-0">
                        ...
                        @else
                            <a href="{{ route('exam.take', $exam->id) }}" class="block w-full md:w-auto px-8 py-3.5 text-center bg-[#4A90E2] hover:bg-[#357ABD] text-white rounded-xl font-semibold text-sm transition-all shadow-lg shadow-[#4A90E2]/30 font-sans">
                                Mulai Kerjakan
                            </a>
                        @endif
                    </div>
                ...
                @empty
                <div class="bg-white p-10 rounded-2xl shadow-sm border border-gray-100 border-dashed flex flex-col items-center justify-center text-center opacity-70 mt-10">
    ...
    <div class="fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 p-3 flex md:hidden justify-around items-center z-50 shadow-[0_-5px_15px_rgba(0,0,0,0.05)] pb-safe">
        ...
        <form action="{{ route('logout') }}" method="POST" class="m-0 p-0 flex">
            ...
            <button type="submit" class="flex flex-col items-center text-red-400 hover:text-red-500 bg-transparent border-none p-0 cursor-pointer transition-colors">
                ...
                <span class="text-[10px] font-medium mt-1 font-sans">Keluar</span>
            </button>
        </form>
    </div>
