...
<head>
    ...
    @vite(['resources/css/app.css', 'resources/js/app.js', 'resources/js/parent/profile.js', 'resources/js/mobile/parent/profile.js'])
</head>
<body class="bg-[#F4F7F6] font-sans flex h-screen overflow-hidden text-[#333]">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-8 items-start">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden relative">
                ...
                <div class="text-center -mt-[60px] relative px-6">
                    <form action="{{ route('profile.ortu.update-avatar') }}" method="POST" enctype="multipart/form-data" id="avatarForm" class="flex flex-col items-center">
                        ...
                        <div id="saveButtonContainer" class="hidden mt-4 flex-col items-center gap-2">
                            <button type="submit" class="px-5 py-2 bg-[#2ECC71] hover:bg-[#27ae60] text-white rounded-full font-semibold text-xs transition-all shadow-sm flex items-center gap-2 cursor-pointer border-none">
                ...
                <div class="text-center px-6 pb-8 pt-4">
                    ...
                    <div class="bg-[#E8F9F5] border border-dashed border-[#2ECC71] p-4 rounded-xl flex items-center gap-4 text-left">
                        <i class="ph-fill ph-link-simple text-2xl text-[#2ECC71] flex-shrink-0"></i>
                        <div class="overflow-hidden">
                            ...
                            @if(Auth::user()->child && Auth::user()->child_connection_status == 'approved') 
                                <strong class="text-[#27ae60] text-sm flex items-center gap-1 truncate"><i class="ph-fill ph-student"></i> {{ Auth::user()->child->name }}</strong>
                                <span class="text-[10px] text-gray-400 block tracking-wider mt-0.5 truncate">ID: {{ Auth::user()->child->user_code }}</span>
                            @elseif(Auth::user()->child && Auth::user()->child_connection_status == 'pending')
                                ...
                                <span class="text-[10px] text-gray-400 block tracking-wider mt-0.5 truncate">{{ Auth::user()->child->name }}</span>
                            @else
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
                    ...
                    @if(!Auth::user()->child)
                        <form action="{{ route('koneksi.connect.ortu') }}" method="POST">
                            ...
                            <div class="flex justify-end mt-6">
                                <button type="submit" class="w-full md:w-auto px-6 py-3 bg-[#4A90E2] hover:bg-[#357ABD] text-white rounded-xl font-semibold text-sm transition-all shadow-lg shadow-[#4A90E2]/30 cursor-pointer flex justify-center items-center gap-2 border-none">
                    ...
                    @elseif(Auth::user()->child_connection_status == 'pending')
                        <div class="relative group">
                            ...
                            <div class="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-1 text-xs font-bold text-[#FF9F43]">
                                <i class="ph-fill ph-clock text-base hidden sm:inline-block"></i> <span class="hidden sm:inline-block">Menunggu Persetujuan</span>
                                <i class="ph-fill ph-clock text-base sm:hidden"></i>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-end mt-6">
                            <form action="{{ route('koneksi.revoke.ortu') }}" method="POST" onsubmit="return confirm('Batalkan permintaan koneksi ini?')" class="w-full md:w-auto">
                                @csrf
                                <button type="submit" class="w-full md:w-auto px-5 py-2.5 bg-white border border-red-200 text-red-500 hover:bg-red-50 rounded-xl font-semibold text-sm transition-all cursor-pointer flex justify-center items-center gap-2">
                    ...
                    @elseif(Auth::user()->child_connection_status == 'approved')
                        <div class="relative group">
                            ...
                            <div class="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-1 text-xs font-bold text-[#2ECC71]">
                                <i class="ph-fill ph-check-circle text-base"></i> <span class="hidden sm:inline-block">Terverifikasi</span>
                            </div>
                        </div>

                        <small class="text-[#27ae60] mt-2 block flex items-start sm:items-center gap-1">
                            <i class="ph-fill ph-shield-check mt-0.5 sm:mt-0"></i> Akun berhasil terhubung dengan {{ Auth::user()->child->name }} (Kelas {{ Auth::user()->child->kelas }}).
                        </small>

                        <div class="flex justify-end mt-6">
                            <button type="button" onclick="openRevokeModal()" class="w-full md:w-auto px-5 py-2.5 bg-white border border-red-200 text-red-500 hover:bg-red-50 rounded-xl font-semibold text-sm transition-all cursor-pointer flex justify-center items-center gap-2">
                ...
                <div class="bg-gray-50 border border-gray-100 p-5 rounded-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    ...
                    <a href="{{ route('profile.ubah-password') }}" class="w-full md:w-auto text-center whitespace-nowrap px-6 py-2.5 bg-white border border-[#4A90E2] text-[#4A90E2] hover:bg-[#F0F7FF] rounded-xl font-semibold text-sm transition-all shadow-sm flex items-center justify-center gap-2 block">
    ...
    <div class="fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 p-3 flex md:hidden justify-around items-center z-50 shadow-[0_-5px_15px_rgba(0,0,0,0.05)] pb-safe">
        <a href="{{ url('/dashboard-ortu') }}" class="flex flex-col items-center text-gray-400 hover:text-[#4A90E2] transition-colors">
            <i class="ph ph-squares-four text-2xl"></i>
            <span class="text-[10px] font-medium mt-1">Home</span>
        </a>
        
        <a href="{{ route('profile.ortu') }}" class="flex flex-col items-center text-[#4A90E2]">
        ...
        <div class="bg-white rounded-2xl shadow-xl w-[90%] max-w-md relative z-10 transform scale-95 opacity-0 transition-all duration-200" id="revokeModalContent">
            <div class="p-6">
                ...
                <div class="bg-gray-50 border border-gray-100 rounded-xl p-4 flex items-center gap-4 mb-8">
                    ...
                    <div class="overflow-hidden">
                        <h4 class="font-bold text-gray-800 text-sm truncate">{{ Auth::user()->child->name }}</h4>
                        <div class="text-xs text-gray-500 mt-1 flex flex-col gap-0.5">
                            <span class="flex items-center gap-1 truncate"><i class="ph-fill ph-identification-card"></i> ID: {{ Auth::user()->child->user_code }}</span>
                            <span class="flex items-center gap-1 truncate"><i class="ph-fill ph-student"></i> Kelas: {{ Auth::user()->child->kelas ?? 'Belum diatur' }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex gap-3">
                    ...
                    <form action="{{ route('koneksi.revoke.ortu') }}" method="POST" class="flex-1">
                        @csrf
                        <button type="submit" class="w-full py-3 bg-red-500 hover:bg-red-600 text-white rounded-xl font-semibold text-sm transition-all shadow-lg shadow-red-500/30 cursor-pointer flex items-center justify-center gap-2 border-none">
